...
export function ItemCard({ type, item }: ItemCardProps) {
    return (
        <article className="group relative flex flex-col overflow-hidden rounded-lg border border-border bg-card transition-colors hover:border-primary">
            <div className="absolute right-2 top-2 z-10"><CompareToggle type={type} item={item} /></div>
            <Link to={`/catalog/${type}/${item.id}`} className="flex h-full flex-col text-foreground no-underline">
                {item.photo ? (
                    <LoadingImage
